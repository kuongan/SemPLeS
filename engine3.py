...
from clip_utils import clip_forward, clip_forward_global, clip_forward_local
...
def train_one_epoch(model: torch.nn.Module, data_loader: Iterable,
    ...
    for samples, targets in metric_logger.log_every(data_loader, print_freq, header):
        ...
        with torch.cuda.amp.autocast():
            ...
            cam_224 = (cam_224 - min_) / (max_ - min_ + 1e-1)
            print(cam_224.max().item(), cam_224.min().item())
            ...
            if model.num_classes == 20:
                ...
                dname = 'coco'
            L_OTM = OTMLoss(clip_forward(clip_model, fg_224_eval, fg_label[fg_indices], dname=dname), 1)

            L_BTM = BTMLoss(clip_forward(clip_model, bg_224_eval, fg_label[fg_indices], dname=dname), 1)

            img_similarity, label_similarity = clip_forward_global(clip_model, fg_224_eval, fg_label[fg_indices], btp.prompts, dname=dname)
            L_Min = BTMLoss(img_similarity, 1)

            loss = L_OTM + hypers[0]*L_BTM + hypers[1]*L_Min
