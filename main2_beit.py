...
from engine2 import train_one_epoch, evaluate, generate_attention_maps_ms
import model_beit
import utils
...
def get_args_parser():
    ...
    parser.add_argument("--btp_length", type=int, default=10)
...
class BTP(nn.Module):
    ...
    def __init__(self, n_class, length, transformer_width):
        super(BTP, self).__init__()

        self.prompts = nn.Parameter(torch.rand(n_class, length, transformer_width))
...
def main(args):
    ...
    btp = BTP(n_class=args.nb_classes, length=args.btp_length, transformer_width=512).cuda()

    linear_scaled_lr = args.lr * args.batch_size * utils.get_world_size() / 512.0
    args.lr = linear_scaled_lr

    optimizer = create_optimizer(args, btp.parameters())
    ...
    for epoch in range(args.start_epoch, args.epochs):
        if args.epoch_save_ckpt and os.path.exists(output_dir / f'checkpoint{epoch}.pth'):
            ...
            train_stats = train_one_epoch(
                ...
                args.clip_grad, clip_model=clip_model, writer=writer, hypers=args.hypers, btp=btp
            )
        ...
        torch.save(btp.state_dict(), args.output_dir + '/btp.pth')
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser('Beit training and evaluation script', parents=[get_args_parser()])
